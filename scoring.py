...
Locked v1 spec encoded here:
  ...
  aggregation       = mean of best 3 result_scores; qualification floor = 3 events
...
MANUAL_MERGES: list[list[str]] = [
    ...
    # Eloise Hild — one sailor, lists "Southern YC / Bay Waveland YC" at
    # gulf_coast and "Bay Waveland YC" alone at midwinters. 1-1 split, blank
    # sail numbers so the frequency guard refused; confirmed one person via
    # whereis.py (Bay Waveland appears in both club strings).
    ["eloise hild|southern yacht club",
     "eloise hild|bay waveland yc"],
]

# Event multipliers now live in events.yaml (single source of truth) and are
# passed into score_event() by run.py. Team Trials 1.25, Nationals 1.15, and the
# rotating Spring Teams Qualifier (stq_status) are set there, not here.
...
CLUB_ALIASES = {
    ...
    "indian harbor yacht club": "indian harbor yacht club",
    "ihyc": "indian harbor yacht club",
    "indian harbor": "indian harbor yacht club",
    "lisot indian harbor yacht club": "indian harbor yacht club",
    
    # ---- batch added from flags.log review, session 2 (verified same-club) ----
    "syc": "southern yacht club",                  # Meade family; full name canonical
    "miles river yacht club": "miles river yc",
    "miles river yc": "miles river yc",
    "south carolina yacht club": "south carolina yc",
    "south carolina yc": "south carolina yc",
    "halifax youth sailing": "haysf",
    "halifax area youth sailing foundation": "haysf",
    "haysf": "haysf",
...
TRAVEL_TEAMS = {
    ...
    "gold winds team",
    "gold winds",
    "goldwin",               # misspelling seen in data
    "bcs",
...
def score_event(payload: dict, event_id: str, event_name: str,
    ...
    for i, r in enumerate(sailed, start=1):
        ...
        # canonical resolved club (empty if travel-team-only) for clubs_seen
        canon_club, _ = resolve_club(r.club)
...
class RankedSailor:
    ...
    sail_numbers: set[str] = field(default_factory=set)
...
def build_ranking(all_event_scores: dict[str, list[EventScore]]) -> tuple[list[RankedSailor], list[str], list[str]]:
    """
    ...
    for event_id, scores in all_event_scores.items():
        for es in scores:
            ...
            rs.name = es.name
            if es.club:
                rs.clubs_seen.add(es.club)
            if es.sail_number:
                rs.sail_numbers.add(es.sail_number)

    # Pass -1: reconcile multi-club splits by event frequency.
    # resolve_club() picks the FIRST real club in a multi-club string, so a
    # sailor whose affiliation is listed in different orders across events
    # (e.g. "Lakewood/SFYC" vs "SFYC/StFYC") splits into multiple keys. Give
    # each name ONE canonical key: the club with the MOST events. A strictly
    # larger count merges freely. An exact TIE at the top is the collision-risk
    # zone (possibly two different same-named sailors), so it merges ONLY if the
    # two keys share a sail number; otherwise it is refused and flagged.
    from collections import defaultdict as _dd
    merge_flags: list[str] = []
    _manual_keys = {k for grp in MANUAL_MERGES for k in grp}
    _real_by_name: dict[str, list[str]] = _dd(list)
    for _key in sailors:
        _nm, _, _club = _key.partition("|")
        if _club:
            _real_by_name[_nm].append(_key)
    for _nm, _keys in _real_by_name.items():
        if len(_keys) < 2:
            continue
        _canon = max(_keys, key=lambda k: (len(sailors[k].scores), k))
        _top = len(sailors[_canon].scores)
        for _k in [x for x in _keys if x != _canon]:
            if len(sailors[_k].scores) == _top:
                _shared = {s for s in (sailors[_canon].sail_numbers & sailors[_k].sail_numbers) if s}
                if not _shared:
                    if _canon in _manual_keys or _k in _manual_keys:
                        continue  # Pass 0 (MANUAL_MERGES) handles this sailor
                    merge_flags.append(
                        f"MERGE? '{_nm}' even split, no sail match: "
                        f"'{_canon}' vs '{_k}' -- review manually")
                    continue
            sailors[_canon].scores.extend(sailors[_k].scores)
            sailors[_canon].clubs_seen |= sailors[_k].clubs_seen
            sailors[_canon].sail_numbers |= sailors[_k].sail_numbers
            del sailors[_k]
    ...
    for key in list(sailors.keys()):
        ...
        if canon and canon != key and key in sailors:
            ...
            target.clubs_seen |= sailors[key].clubs_seen
    ...
    for key, rs in list(sailors.items()):
        ...
        if club_part == "":  # this is a name-only (travel-team) entry
            ...
            if len(candidates) == 1:
                ...
                target.clubs_seen |= rs.clubs_seen
    ...
    dup_warnings = find_split_survivors(sailors)
    ...
    return qualified, merge_flags, dup_warnings


def find_split_survivors(sailors: dict) -> list[str]:
    """
    POST-MERGE duplicate report. Runs on the FINAL sailors dict (after the
    frequency pass, Pass 0, and Pass 2), so it only reports names that STILL
    map to 2+ distinct keys — genuine unresolved same-name ambiguities, not
    splits the passes already fixed. This is the report you act on.
    """
    from itertools import combinations
    by_name: dict[str, dict] = {}
    for key, rs in sailors.items():
        name_part, _, club_part = key.partition("|")
        by_name.setdefault(name_part, {})[club_part] = rs.name
    ...
    for name_part, club_map in by_name.items():
        if len(club_map) > 1:
            ...
            for a, b in combinations(clubs, 2):
                pairs.setdefault((a, b), []).append(display)
    if not pairs:
        return []
    ranked = sorted(pairs.items(), key=lambda kv: len(kv[1]), reverse=True)
    lines = []
    total = sum(1 for n, cm in by_name.items() if len(cm) > 1)
    lines.append(f"=== {total} UNRESOLVED same-name split(s) post-merge "
                 f"(review: one person or two?) ===")
    for (a, b), names in ranked:
        uniq = sorted(set(names))
        sample = ", ".join(uniq[:6]) + (" ..." if len(uniq) > 6 else "")
        lines.append(f"[{len(uniq):>3}] '{a}'  <->  '{b}'   e.g. {sample}")
    return lines
